...
4. Create or validate the annotated `v<version>` tag at that same commit. Confirm `.github/workflows/npm-publish.yml` still triggers on `v*.*.*` tag pushes, blocks publication on both exact annotated-tag/main/package identity and `tools/release-prep.mjs --check-only --version "${GITHUB_REF_NAME#v}"`, and uses npm trusted publishing.
   - 同じコミットに注釈付きの `v<version>` タグを作成または検証します。`.github/workflows/npm-publish.yml` が引き続き `v*.*.*` タグのリモート送信で起動し、注釈付きタグ・main・package の正確な同一性と `tools/release-prep.mjs --check-only --version "${GITHUB_REF_NAME#v}"` の両方を公開ブロッカーとして強制し、npm trusted publishing を使用することを確認します。
5. Before any release-tag push, confirm the publication gateway prerequisites summarized in [Release Publication Security](../../docs/development/release-security.md): `.github/workflows/npm-publish.yml` binds its publish job to the exact `npm-publish` environment; npm Trusted Publisher is configured for the exact repository, workflow filename `npm-publish.yml`, and environment `npm-publish`; the GitHub Environment has the intended required reviewers and deployment ref rules; and a GitHub ruleset protects creation of `v*` release tags. Repository code cannot verify these external settings. If the release agent cannot independently observe every setting, stop before pushing the tag and ask the maintainer to confirm them. Do not attempt to create or change GitHub or npm security settings unless separately authorized.
   - リリースタグをリモートへ送信する前に、[Release Publication Security](../../docs/development/release-security.md) に要約された公開ゲートの前提条件を確認します。`.github/workflows/npm-publish.yml` の公開用 job が正確な `npm-publish` environment に結び付けられていること、npm Trusted Publisher が正確なリポジトリ、workflow filename `npm-publish.yml`、environment `npm-publish` に設定されていること、GitHub Environment に意図した required reviewers と deployment ref rules があること、および GitHub ruleset が `v*` リリースタグの作成を保護していることを確認します。タグのリモート送信には、step 6 に定めるメンテナーの明示的承認後にのみ実行できる条件（`only after explicit maintainer approval`）が引き続き適用されます。リポジトリコードでは、これらの外部設定を検証できません。リリースエージェントがすべての設定を独立して確認できない場合は、タグをリモートへ送信する前に停止し、メンテナーに確認を求めます。別途承認されない限り、GitHub または npm のセキュリティ設定を作成または変更しません。
...
7. Only after the separate tag-push approval may the workflow reach the `npm-publish` GitHub Environment approval gate. Monitor that workflow through completion, and treat a failed test, build, package check, exact tag/main/package identity check, deterministic release metadata check, or publication step as a release blocker. Environment approval may legitimately leave the publication job waiting; report that state as pending and wait for an authorized reviewer rather than bypassing the gate.
   - step 6 に定めるメンテナーの明示的承認後にのみ（`only after explicit maintainer approval`）タグをリモートへ送信し、その後ワークフローは `npm-publish` GitHub Environment の承認ゲートへ進めます。起動したワークフローを完了まで監視し、テスト、ビルド、パッケージチェック、タグ・main・package の正確な同一性チェック、決定的なリリースメタデータチェック、または公開手順の失敗をリリースブロッカーとして扱います。Environment の承認待ちにより公開ジョブが待機状態になることがあります。その状態を保留中として報告し、ゲートを回避せず、権限を持つレビュー担当者を待ちます。
...
- Treat the external publication gateway as mandatory before seeking the separate approval for a release-tag transfer to GitHub. Do not infer its configuration from repository files, and do not create or change GitHub or npm security settings without separate authorization.
