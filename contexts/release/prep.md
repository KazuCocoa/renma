...
7. Only after the separate tag-push approval may the workflow reach the `npm-publish` GitHub Environment approval gate. Monitor that workflow through completion, and treat a failed test, build, package check, tag/version check, or publication step as a release blocker. Environment approval may legitimately leave the publication job waiting; report that state as pending and wait for an authorized reviewer rather than bypassing the gate.
   - step 6 に定めるメンテナーの明示的承認後にのみ（`only after explicit maintainer approval`）タグをリモートへ送信し、その後ワークフローは `npm-publish` GitHub Environment の承認ゲートへ進めます。起動したワークフローを完了まで監視し、テスト、ビルド、パッケージチェック、タグとバージョンのチェック、または公開手順の失敗をリリースブロッカーとして扱います。Environment の承認待ちにより公開ジョブが待機状態になることがあります。その状態を保留中として報告し、ゲートを回避せず、権限を持つレビュー担当者を待ちます。
