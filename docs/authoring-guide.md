...
Run `renma validate-skills .` to validate canonical Skills with real YAML 1.2 parsing. Invalid quoting, escapes, collection shapes, field types, or duplicate keys are specification failures. Constraint prominence is section-specific: a constraint under `Procedure` remains buried even if another `Hard Constraints` heading exists elsewhere.

Agent Skills names come from valid immediate parent-directory names and are not silently slugified. Rename an invalid directory before scaffolding or migration. If canonical `metadata.renma.*` and legacy top-level metadata conflict, `suggest-metadata` preserves both sources and blocks canonical frontmatter until a human resolves the semantic value. Canonical security values are used for deterministic evaluation and replace, rather than union with, conflicting legacy lists.
