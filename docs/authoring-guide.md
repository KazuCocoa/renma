...
Run `renma validate-skills .` to validate canonical Skills with real YAML 1.2 parsing. The same parsed YAML tree supplies metadata to catalog, ownership, relationships, lifecycle, and security. Invalid quoting, escapes, collection shapes, field types, or duplicate keys are specification failures. Constraint prominence follows heading ancestry: subsections under `Hard Constraints` remain prominent, while a constraint under `Procedure` remains buried even if another constraint heading exists elsewhere.

Starting with Renma 0.16.0, Agent Skills-compatible frontmatter is the only operational Skill format. Historical top-level fields are read only by the one-way migration assistant; legacy and hybrid Skills remain invalid. Agent Skills names come from valid immediate parent-directory names and are not silently slugified. Rename an invalid directory before scaffolding or migration, and provide `--id` when a Unicode path cannot produce a unique non-lossy Renma ID. If canonical `metadata.renma.*` and historical top-level metadata conflict, `suggest-metadata` preserves both sources and blocks canonical frontmatter until a human resolves the semantic value. Unknown `renma.*` and other vendor metadata are preserved.
