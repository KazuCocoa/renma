...
| Severity  | Message                                                                                           | Meaning                                                                                                                        | Fix                                                                                                             |
| --------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| `warning` | `Invalid status "<status>". Expected one of: experimental, stable, suspended, deprecated, archived.` | An asset status does not match the accepted status values.                                                                  | Replace the status with a supported value.                                                                      |
| `error` or `warning` | `Invalid status_changed_at "<date>". Expected a real ISO calendar date in YYYY-MM-DD format.` | Lifecycle transition evidence has an invalid date; this is blocking while status is suspended and a warning otherwise. | Confirm and record the real reviewed transition date; do not infer it from Git or file timestamps. |
...
ordinary variable interpolation, does not establish that operation by itself.
One narrow generated-script case is correlated within the same bounded logical
shell instruction: proven static `echo`/`printf` text must be written by stdout
redirection or a proven literal pipeline through `tee` to an exact static path,
and a later modeled shell or direct current-shell `source`/`.` command must
consume that path. The generated bytes are split into physical lines, blank and
comment/shebang lines are skipped, backslash continuations are projected, and
each resulting logical command is classified independently by the existing
command-position model without recursively correlating another generated
script. The consumer may be followed by `|| true` or `|| :`.

Generated-file facts cross only proven non-mutating commands. Exact effects from
redirections, `tee`, bounded `cp`, `mv`, `install`, `rm`, `unlink`, `truncate`,
and in-place `sed` forms invalidate or replace affected facts; an unmodeled
command invalidates all such proofs. Direct `cd`, `pushd`, and `popd` invalidate
relative facts while unaffected absolute facts survive. `;` and `&&` carry the
...
or filesystem identity, correlate separate instructions, or infer execution
from an unmatched write. Command substitution remains operational. A separate
bounded prose grammar covers both “If the destructive operation fails, ignore
the error and continue” and the coordinated reverse order “continue and ignore
the error.” Generic `|| true`,
...
| `META-INVALID-STATUS-CHANGED-AT`                 | Lifecycle transition date is invalid.                | `status_changed_at` is present but is not a real `YYYY-MM-DD` date; severity is error for suspended status and warning otherwise. | Confirm the real reviewed transition date; do not derive it from Git or filesystem timestamps. |
