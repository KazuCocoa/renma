...
| `QUAL-INVALID-TOKEN-BUDGET-OVERRIDE`             | Support-asset override metadata is invalid.           | The override is not a positive integer above the default, lacks a rationale, or has an invalid optional review date. | Correct or remove malformed metadata. Ask about a meaningful split first; use an override only after the user confirms the asset should remain intentionally long. |
| `QUAL-SUPPORT-ASSET-TOKEN-BUDGET`                | Support asset exceeds its effective advisory estimate. | A context, reference, profile, or example exceeds its default or valid reviewed override.          | Ask whether a semantic split preserves coherence and execution order. Split only with user agreement; otherwise record an explicit human rationale, never an override added merely to pass diagnostics. |
