...
| `META-DEPENDENCY-TARGET-KIND-MISMATCH`           | A resolved relationship targets the wrong asset kind. | For example, `requires_context` points to a Context Lens, `requires_lens` points to Context, or `applies_to` is authored outside a Lens or points outside Context. | Correct the target or declaration using repository intent; do not create placeholders or reject valid Context-to-Context dependencies. |
