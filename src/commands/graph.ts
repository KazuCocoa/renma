...
function executableGraphReport(
  report: GraphReport,
  ...
  const sourceNode = (sourcePath: string): GraphNode | undefined => {
    const surface = surfacesByPath.get(sourcePath);
    if (surface) return surfaceNode(surface, report.nodes);
    ...
    return [
      {
        ...
        evidenceCount: inventory.invocations.filter(
          (invocation) =>
            invocation.sourcePath === edge.sourcePath &&
            invocation.normalizedTarget === edge.normalizedTarget &&
    ...
    return [
      {
        ...
        targetKind: "script",
        targetPath: target.path,
        evidenceCount: inventory.dependencies.filter(
          (dependency) =>
            dependency.sourcePath === edge.sourcePath &&
            dependency.normalizedTarget === edge.normalizedTarget &&
  ...
  const surfaceNodes = inventory.surfaces.map((surface) => {
    const node = surfaceNode(surface, report.nodes);
    const invokedBySkillCount = invokedBySkills.get(surface.path)?.size ?? 0;
    return {
      ...node,
      invokedBySkillCount,
    } satisfies GraphNode;
...
function surfaceNode(
  ...
  catalogNodes: readonly GraphNode[],
): GraphNode {
  const catalogNode = catalogNodes.find(
    (node) => node.sourcePath === surface.path,
  );
  return {
    ...(catalogNode ?? {
      id: surface.path,
...
export function formatGraphMermaid(
  ...
  if (view === "executable") return formatExecutableMermaid(report);
...
export function formatGraphMarkdown(
  ...
  if (view === "executable") return formatExecutableMarkdown(report);
...
function formatExecutableMarkdown(report: GraphReport): string {
...
function formatExecutableMermaid(report: GraphReport): string {
