...
import type { Artifact, ArtifactKind, Diagnostic, ScanConfig } from "./types.js";

export async function discoverArtifacts(root: string, config: ScanConfig): Promise<{ artifacts: Artifact[]; diagnostics: Diagnostic[] }> {
  const diagnostics: Diagnostic[] = [];
  const paths = new Set<string>();

  for (const pattern of config.globs) {
    try {
      for await (const match of glob(pattern, { cwd: root, withFileTypes: false })) {
        if (typeof match === "string") paths.add(toPosix(match));
      }
    } catch (error) {
      diagnostics.push({ severity: "error", message: `Could not evaluate glob "${pattern}": ${errorMessage(error)}` });
    }
  }
  ...
  const artifacts = await mapLimit(candidates, config.concurrency, async (relativePath) => {
    ...
    try {
      ...
      if (linkInfo.isSymbolicLink()) {
        diagnostics.push({ severity: "warning", path: relativePath, message: "Skipping symbolic link." });
        return undefined;
      }
      ...
      diagnostics.push({ severity: "error", path: relativePath, message: `Could not read file: ${errorMessage(error)}` });
      return undefined;
    }
  });

  return { artifacts: artifacts.filter((artifact): artifact is Artifact => artifact !== undefined), diagnostics };
}
...
function classify(relativePath: string): ArtifactKind {
  ...
  if (/\/tasks\/[^/]+\.(?:json|ya?ml)$/.test(relativePath)) return "eval_task";
  if (
...
function isExcluded(relativePath: string, excludes: string[]): boolean {
  ...
  return excludes.some((exclude) => segments.includes(exclude) || relativePath === exclude || relativePath.startsWith(`${exclude}/`));
}
