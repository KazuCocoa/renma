...
export const COMMAND_HELP = [
  ...
  {
    name: "guide",
    usage: "renma guide <topic> [options]",
    question: "What is the smallest justified asset graph for a new Skill?",
    purpose:
    ...
    doNotUseFor: [
      "Conducting the conversation, asking the user questions directly, retaining session state, accepting a natural-language task, calling an LLM, or designing the Skill automatically.",
      "Creating or editing files, fetching URLs, or inspecting a repository.",
      "Inferring owners, policies, dependencies, domain facts, or runtime Context selection.",
      "Claiming scan creates Context Assets or proves semantic correctness and source authority.",
    ],
    examples: [
      "renma guide skill",
    ...
    interpretation: [
      ...
      "The creation gate may be declared passed only after every gate requirement is established, including the smallest justified asset structure, and no Blocking authoring decision remains. This is an observable dependency, not a prescribed internal reasoning sequence.",
      ...
      "Classify unknown scope before progression: authoring decisions may block the gate, while runtime task unknowns should be detected, reported with evidence and impact, or treated as runtime-stage blockers under the Skill's authored ask, report, defer, or stop policy when the next execution stage depends on them. A task-instance fact does not enter the authoring blocker set; re-enter clarification only when its handling policy or asset boundary is unresolved.",
      "Decision themes and disposition summaries are optional working aids. Preserve evidence and the distinction between blocking decisions, reversible defaults, deferred decisions, and runtime findings without requiring a particular reasoning sequence or progress format.",
    ...
    nextSteps: [
      "Investigate relevant evidence, evaluate the creation gate, clarify only unresolved Blocking human truth when needed, and retain unresolved decisions instead of inventing or hiding answers.",
      "Establish the smallest non-redundant asset graph as a gate requirement; declare the gate passed only after every requirement is established and no Blocking authoring decision remains, then record a renma.skill-authoring-handoff.v1 exchange artifact.",
    ...
    options: [
      {
        name: "format",
        description: "Output format: prompt or json. Defaults to prompt.",
      },
      "json",
      "help",
    ],
  ...
  },
  {
    ...
    interpretation: [
      ...
      "For Skills, metadata review is only one part of authoring review; use platform-native Skill authoring guidance for the trigger description, instructions, workflow, constraints, and completion criteria.",
    ],
    nextSteps: [
...
export function renderGlobalHelp(version: string): string {
  return [
    ...
    "  rerun renma scan . --fail-on high and review the result",
    "  use renma guide skill only when intentionally reconsidering asset boundaries",
    "",
    "Start here: new skill",
    "  renma guide skill",
    "  let an external LLM investigate and clarify only unresolved Blocking authoring decisions when needed",
    "  establish every creation-gate requirement, including the smallest intended asset structure",
    "  declare the gate passed only after those requirements are established and no Blocking authoring decision remains",
    "  record renma.skill-authoring-handoff.v1 with that established structure",
    ...
    "  renma catalog . --format markdown",
    "  renma graph . --format markdown",
    "  fix and rerun, then require human review",
    "",
